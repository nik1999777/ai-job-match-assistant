...
│                    FASTAPI (port 8000)                          │
│                                                                 │
│  /api/analyze        →  резюме (текст/hh.ru/PDF) +               │
...
│  /api/fetch-vacancy →  URL → текст вакансии (API + fallback)    │
│  /api/batch         →  batch анализ N резюме × 1 вакансия       │
│                         mode=hr, asyncio.gather, макс 20        │
...
Playwright — fallback для случаев когда API недоступен (региональные субдомены, DDoS Guard).
...
Неделя 3 (DONE ✅):
  frontend/                      — React 19 + Vite 8 + Zustand + TanStack Query + Orval
    ...
    components/ModeToggle.tsx    — переключатель Соискатель / HR
    ...
    pages/AnalysisPage.tsx       — seeker режим
    pages/HRBatchPage.tsx        — hr режим (batch анализ)
  api/llm/streaming.py           — +parsed_data и +gap_data SSE события, +mode param
