...
│   └── вариант B: пользователь даёт URL → Playwright достаёт страницу
│
└── Резюме пользователя
...
hh.ru защищён DDoS Guard — `api.hh.ru` возвращает 403 без браузерной сессии. Playwright запускает headless Chromium: выполняет JavaScript, сохраняет cookies DDoS Guard, ведёт себя как реальный пользователь. Публичная страница вакансии (`hh.ru/vacancy/{id}`) доступна без авторизации.

**Почему нельзя парсить резюме?**  
hh.ru закрывает резюме — `GET /resumes/{id}` требует OAuth именно того человека.

#### `api/clients/hh_client.py` — Playwright клиент к hh.ru
...
_page_to_vacancy_data(page)    → dict           # извлечь из открытой страницы
```

Извлечение данных из страницы — два уровня:
1. `<script type="application/ld+json">` с `@type: JobPosting` — Schema.org, самый надёжный
2. DOM fallback: `data-qa="vacancy-title"`, `data-qa="vacancy-description"`, `data-qa="bloko-tag__text"`
...
Неделя 3 (DONE ✅ частично):
  frontend/                    — React 19 + Vite 8 + shadcn/ui + FSD архитектура
