...
| Инференс open-source LLM | Groq-провайдер (llama-4-scout / 70b / 8b) + Ollama-провайдер (llama3) | `api/llm/provider.py` |
...
│  /api/parse-resume → PDF upload → PyMuPDF → {text, file_id}    │
│  /api/resumes/{file_id} → скачать оригинальный PDF (JWT req.)  │
...
│             │   │                                              │
│  Groq       │   │  SkillExtractor     SeniorityClassifier      │
│  llama-4-   │   │  (BERT NER)         (DistilBERT + LoRA)      │
│  scout (def)│   │                      junior/mid/senior       │
│     OR      │   │  SkillMatcher                                │
│  OpenAI     │   │  exact match +      Qdrant Retriever         │
│  GPT-4o-mini│   │  BAAI/bge cosine    hybrid search            │
│  (prod)     │   │  similarity         top-k vacancies          │
│     OR      │   │                                              │
│  Ollama     │   │  LanguageDetector   Qdrant Indexer           │
│  (dev only) │   │  Cyrillic ratio     dense(BAAI)+sparse(BM42) │
│             │   └──────────────────────────────────────────────┘
└─────────────┘
       │                  │
       └──────┬───────────┘
...
POST /api/analyze
{
  "resume": "Опыт: 3 года Python, FastAPI...",   // текст резюме (из PDF upload)
  "resume_file_id": "a1b2c3d4-...",              // UUID сохранённого PDF (для скачивания из истории)

  // вакансия: один из вариантов
  "vacancy_url": "https://hh.ru/vacancy/12345",   // hh.ru URL (сохраняется → ссылка в истории)
...
1. Берётся текст резюме (уже распарсен клиентом через `/api/parse-resume`)
...
4. Создаётся `Analysis` (resume_text, resume_file_id, vacancy_text, vacancy_url) — FK на Session
...
### Шаг 4 — advise_node (LLM структурированный совет)
...
**Structured output** через `with_structured_output()` — LLM возвращает строгую Pydantic-схему, не свободный текст:

```python
class SkillTip(BaseModel):
    skill: str       # название навыка
    action: str      # 1 предложение: как изучить

class SeekerAdvice(BaseModel):  # mode=seeker
    overall: str             # общая оценка совместимости
    top_skills: list[SkillTip]  # 3 навыка с конкретным action
    resume_tips: list[str]   # правки в резюме
    strategy: str            # стратегия подачи заявки

class HRAdvice(BaseModel):   # mode=hr
    candidate_fit: str       # 1-2 предложения
    strengths: list[str]
    gaps: list[str]
...
**Почему structured output, а не свободный текст?**  
Свободный текст → стена markdown, которую неудобно читать. Structured output → UI рендерит каждое поле отдельным блоком: skill badges, progress-bars, decision chip.  
Pydantic-схема с `Field(description=...)` инструктирует LLM точно так же как промпт — без отдельного system message.  
`result.model_dump()` → JSON → `llm_response` в БД (backward compatible, старые записи рендерятся как plain text).
...
data: {"event": "node_start",  "node": "advise_node"}
  # structured output — нет потокового текста; ждём пока LLM завершит всю схему
data: {"event": "advice_data", "data": {"overall": "...", "top_skills": [...], ...}}
...
get_llm()  →  ChatGroq(llama-4-scout)       # если LLM_PROVIDER=groq  (default)
           →  ChatOpenAI(gpt-4o-mini)       # если LLM_PROVIDER=openai
           →  ChatOllama(llama3)            # если LLM_PROVIDER=ollama
```

**Три провайдера:**
- **Groq** — бесплатный tier с отдельными квотами на каждую модель. Прокси через httpx.AsyncClient (только трафик Groq), чтобы hh.ru / HuggingFace / Ollama шли напрямую. Default: `llama-4-scout-17b-16e-instruct` — Mixture of Experts, лучший результат eval.
- **OpenAI** — production (GPT-4o-mini): нативный function calling, надёжный structured output
- **Ollama** — локальная разработка без интернета и затрат. В prod не используется.

Все реализуют `BaseChatModel` из LangChain — остальной код не меняется.

**Groq из РФ:** streaming endpoint заблокирован. Решение — `GROQ_PROXY=http://user:pass@host:port` в `.env` + запуск сервера без системного прокси: `env -u HTTP_PROXY -u HTTPS_PROXY uvicorn api.main:app --port 8000`

**Настройка через `.env`:**
- `LLM_TEMPERATURE=0.0` — детерминированные ответы (обязательно для eval)
- `RESUME_CONTEXT_LIMIT` / `VACANCY_CONTEXT_LIMIT` — лимиты контекста под модель: `8b/scout=8000/2000`, `gpt-4o=20000/4000`
- `GROQ_MODEL` — переключение модели без перезапуска кода. Все модели: [.env.example](.env.example)

**Groq модели (eval результаты, 12 кейсов):**

| Модель | match_in_range | MAE | skill_f1 | latency | TPD |
|---|---|---|---|---|---|
| llama-4-scout-17b (MoE) | **92%** | **0.004** | 0.772 | 3727ms | ~500K |
| llama-3.3-70b-versatile | 83% | 0.054 | 0.766 | 4341ms | 100K |
| llama-3.1-8b-instant | 67% | 0.300 | 0.810 | 16983ms | 500K |
...
│   ├── resume_file_id (str | null — UUID → uploads/resumes/{uuid}.pdf)
│   ├── vacancy_url (str | null — hh.ru URL, сохраняется из /api/analyze)
...
Stage 2: cosine similarity via BAAI/bge-small-en-v1.5
  ...
  Калибровка порога на реальных парах (data-driven):
    numpy ≈ TensorFlow   = 0.725  → false positive → должен быть ниже порога
    LangChain ≈ LangGraph = 0.767  → true match   → должен быть выше порога
    GitLab CI ≈ GitLab   = 0.845  → true match   ✓
    Postgres ≈ PostgreSQL = 0.934  → true match   ✓
  Порог 0.75 — единственное значение, при котором false positive убирается, а все true match сохраняются.
...
│
└── Резюме пользователя
    └── PDF upload → POST /api/parse-resume → PyMuPDF → {text, file_id}
          оригинальный PDF → uploads/resumes/{uuid}.pdf (не в git)
          GET /api/resumes/{file_id} + Bearer JWT → FileResponse (скачать из истории)
```
...
Payload каждой точки:
  vacancy_id, title, company, skills[], text_preview (500 chars), url, salary_str
...
Возвращаемые поля: `vacancy_id, title, company, skills, score, url, salary_str` — все поля попадают во `similar_vacancies` в state, SSE и БД.
...
```bash
# Cold-start: 46 специализаций × 100 вакансий (~4600 вакансий, ~2 часа):
...
**46 специализаций:** AI/ML/LLM, Python/Java/Go/Node.js/PHP/C#/Kotlin/Ruby backend, React/Vue/Angular/TypeScript frontend, iOS/Android/Flutter/React Native mobile, DevOps/SRE/Cloud, Data Engineer/Analyst/BI, QA, InfoSec, Embedded, C++, Solution Architect, Tech Lead, Team Lead, PM, UX/UI/Product Designer.

**3-tier skill extraction (приоритет по убыванию):**
1. `key_skills` из hh.ru API/DOM — официальные теги от HR, нормализованы самим hh.ru
2. BERT NER (`dslim/bert-base-NER`) — ловит навыки из описания, которые HR не проставил в тегах
3. `_SKILL_KEYWORDS` regex — fallback для вакансий без тегов

**Стратегия получения данных:** Playwright для search (API требует OAuth2), hh.ru API для деталей вакансии (salary, key_skills, company) + Playwright fallback. Payload в Qdrant: title, company, skills, url, salary_str, text_preview.

`upsert` идемпотентен — повторный запуск безопасен. `area=113` (вся Россия) + `no_magic=true` предотвращают гео-редирект.
...
- После завершения → `trace.score()`: match_score, seniority_confidence, latency_s, skills_missing_count (Scores tab)
- Output трейса: match_score, seniority, seniority_confidence, vacancy_seniority_hint, skills_missing
- Metadata: vacancy_seniority_hint — видно был ли применён penalty и почему
...
Неделя 4 (DONE ✅) — Eval pipeline + Observability:
  ...
  api/llm/streaming.py           — Langfuse direct client (не LangChain callback):
                                   ...
                                   output/metadata: vacancy_seniority_hint (анализ penalty)
...
Неделя 4 (DONE ✅) — Groq provider + промпт-инжиниринг + модельное сравнение:
  api/llm/provider.py            — Groq провайдер с per-client httpx proxy
                                   (только Groq трафик через прокси; hh.ru/HuggingFace — напрямую)
  api/llm/streaming.py           — groq/ prefix для model name в Langfuse
  api/agents/nodes/parse.py      — _SKILL_RULES shared constant (few-shot в Field descriptions)
                                   vacancy_seniority_hint: str + field_validator(mode="before")
                                   вместо Literal (LangChain валидирует до Pydantic → падало)
                                   OR-условия → включаем ВСЕ варианты в vacancy_skills
  api/agents/nodes/gap.py        — NER дополняет только resume_skills (не vacancy)
                                   vacancy_skills только из LLM (NER загрязнял ORG/MISC сущностями)
  api/ml/skill_extractor.py      — tail chunk обработка: если текст > MAX_CHARS → обрабатываем
                                   начало и конец (находит скиллы в конце длинных резюме)
  api/routes/seek.py             — settings.resume_context_limit вместо smart_truncate_resume
  api/settings.py                — groq_api_key, groq_model, groq_proxy
                                   default: meta-llama/llama-4-scout-17b-16e-instruct
  .env.example                   — полная таблица Groq моделей: TPD лимиты, quality, примечания
  README.md                      — Quick Start блок + таблица провайдеров + Groq модели

Неделя 4 (DONE ✅) — Code review + backend quality fixes:
  api/routes/batch.py            — _LLM_SEM(3): batch тоже запускал N резюме без семафора
                                   → TPM rate limit → score=0 для последних кандидатов. Исправлено.
  api/routes/seek.py             — logging.getLogger добавлен; exception не течёт клиенту
  api/routes/batch.py + seek.py  — match_score_hire_threshold / match_score_consider_threshold
                                   вынесены из захардкоженных 0.75/0.50 в settings.py
  api/llm/streaming.py           — @cache на _langfuse_client() → singleton вместо нового
                                   HTTP-клиента на каждый запрос /api/analyze
  api/ml/skill_matcher.py        — skill_match_threshold 0.72 → 0.75 (data-driven калибровка):
                                   numpy≈TensorFlow=0.725 убран (false positive)
                                   LangChain≈LangGraph=0.767 сохранён (true match)
  eval/dataset.py                — кейс #04 диапазон (0.65, 0.95) → (0.65, 1.00):
                                   score=1.0 корректен при OR-условии (GitLab или GitHub Actions)

Результаты eval (2026-05-28, 12 кейсов, Llama 4 Scout + threshold 0.75):
  match_in_range: 12/12 (100%) ← идеал  |  MAE: 0.000 ← идеал
  skill_recall: 0.917  |  skill_precision: 0.779  |  F1: 0.817
  seniority_accuracy: 5/12 (42%) ← не зависит от LLM, нужен LoRA fine-tuning
  avg_latency: 3303ms

Сравнение моделей (threshold=0.75):
  Ollama llama3 (baseline) : 83%  MAE=0.017  F1=0.543  (медленно)
  Groq llama-3.3-70b       : 83%  MAE=0.054  F1=0.766  (4341ms, 100K TPD)
  Groq llama-3.1-8b        : 67%  MAE=0.300  F1=0.810  (16983ms, 500K TPD)
  Groq llama-4-scout-17b   : 100% MAE=0.000  F1=0.817  (3303ms, ~500K TPD) ← BEST

Неделя 4 (DONE ✅) — UX улучшения + PDF хранение:
  api/routes/parse_resume.py     — сохраняет оригинальный PDF: uploads/resumes/{uuid}.pdf
                                   возвращает {text, file_id} вместо просто {text}
  api/routes/resumes.py          — новый маршрут: GET /api/resumes/{file_id} (JWT required)
                                   ownership check: Analysis.resume_file_id + Session.user_id
                                   FileResponse(uploads/resumes/{uuid}.pdf)
  api/db/models.py               — Analysis: +resume_file_id (str | null), +vacancy_url (str | null)
  api/routes/analyze.py          — AnalyzeRequest: +resume_file_id, +vacancy_url → сохраняются в Analysis
  api/routes/history.py          — AnalysisDetail ответ: +resume_file_id, +vacancy_url
  api/main.py                    — include_router(resumes.router)
  alembic/versions/d30c80467fd3  — миграция: add vacancy_url to analyses
  alembic/versions/40fa45ae75c9  — миграция: add resume_file_id to analyses
  .gitignore                     — uploads/ (PDF файлы не в git)
  frontend/components/PdfFileCard.tsx   — упрощена: нет предпросмотра текста; кликабельное имя + ✓ + [✎]
  frontend/components/TextContentCard.tsx — упрощена: однострочная карточка без предпросмотра
                                           иконка + метка + кол-во симв. + ✓ + опциональный [✎]
  frontend/widgets/AnalyzeForm.tsx      — PDF-only дропзона (нет вкладки "Текст")
                                         URL-only вакансия + ↗ ссылка; resume_file_id → /api/analyze
  frontend/widgets/SeekForm.tsx         — PDF-only дропзона (нет вкладки "Текст")
  frontend/widgets/BatchForm.tsx        — вакансия: onBlur auto-fetch (убрана кнопка "Загрузить")
                                         после fetch → TextContentCard (без кнопки подтверждения)
  frontend/hooks/useUploadResume.ts     — callback(text, fileId | null) вместо callback(text)
  frontend/api/generated.ts             — AnalyzeRequest: +resume_file_id, +vacancy_url
  frontend/hooks/useHistory.ts          — AnalysisDetail: +resume_file_id, +vacancy_url типы
  frontend/pages/AnalysisDetailPage.tsx — нет сырых текстовых блоков
                                         ↗ вакансия (hh.ru ссылка) + ⬇ Скачать резюме (PDF) вверху
                                         fetch /api/resumes/{file_id} + Bearer → blob → resume.pdf
  frontend/pages/BatchDetailPage.tsx    — убран блок с текстом вакансии

Неделя 5 (DONE ✅) — Structured advice + Qdrant enrichment + UX:
  api/agents/nodes/advise.py     — structured output: SeekerAdvice / HRAdvice Pydantic-схемы
                                   with_structured_output() → result.model_dump() → JSON в БД
                                   backward compatible: старые текстовые записи рендерятся как plain text
  api/llm/streaming.py           — advice_data SSE событие вместо token-стриминга
                                   on_chain_end advise_node → {"event":"advice_data","data":{...}}
  api/rag/indexer.py             — +company, +salary_str параметры → хранятся в Qdrant payload
  api/rag/retriever.py           — возвращает company, salary_str из payload
  api/agents/nodes/gap.py        — similar_vacancies включают company, salary_str
  api/db/models.py               — Analysis: +similar_vacancies (Text, JSON — top-3 похожих вакансий)
  alembic/versions/e4f5a6b7c8d9  — миграция: add similar_vacancies to analyses
  api/routes/analyze.py          — сохраняет similar_vacancies = json.dumps(similar) в Analysis
  api/routes/history.py          — AnalysisDetail: +similar_vacancies list[dict]
  scripts/index_vacancies.py     — 46 специализаций (был 16 ML-only)
                                   3-tier extraction: key_skills (hh.ru) + BERT NER + regex
                                   +company и +salary_str → в Qdrant payload
  frontend/store/analysisStore.ts — SimilarVacancy: +company, +salary_str
                                    AdviceData = SeekerAdvice | HRAdvice (вместо tokens: string)
  frontend/api/streaming.ts      — обрабатывает advice_data вместо token событий
  frontend/hooks/useAnalyze.ts   — onAdviceData → store.setAdviceData
  frontend/hooks/useHistory.ts   — SimilarVacancy type: +company, +salary_str
  frontend/components/AdviceCard.tsx — новый компонент: рендер SeekerAdvice / HRAdvice
                                   isSeekerAdvice() type guard; AdviceSkeleton во время загрузки
  frontend/components/SimilarVacancies.tsx — новый компонент:
                                   список вакансий: title + company + salary + ↗ ссылка
                                   skill benchmark: bar chart частоты навыков, красный = missing
  frontend/widgets/AnalysisResult.tsx — убран PipelineInspector; добавлен SimilarVacancies блок
                                   AdviceSkeleton пока advise_node работает
  frontend/pages/AnalysisDetailPage.tsx — SimilarVacancies блок между skills и advice
                                   parseAdvice(): JSON → AdviceCard, fallback → plain text
  frontend/ (все компоненты)     — полная русификация: все user-facing тексты на русском
                                   DecisionBadge, NODE_LABELS, DECISION_LABELS, пустые состояния
