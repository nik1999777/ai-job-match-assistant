...
Резюме + фильтры (query, area, experience, salary, remote, count)
  ...
  │       → возвращает VacancyItem[] без доп. API-вызовов
  │     ─ HHOAuthSearchProvider (будущее): api.hh.ru/vacancies с токеном
  │     ─ LinkedInSearchProvider (будущее): LinkedIn Jobs API или Apify
...
### 4.6 Слой данных — hh.ru (Playwright)
...
Неделя 3 (DONE ✅):
  ...
  api/clients/vacancy_search.py  — VacancySearchProvider Protocol + HHPlaywrightSearchProvider
                                   + HHOAuthSearchProvider (stub) + LinkedInSearchProvider (stub)
  ...
  api/routes/seek.py             — POST /api/seek SSE: resume + filters → ranked vacancies
