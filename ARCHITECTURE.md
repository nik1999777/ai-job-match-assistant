...
> Портфолио-проект для позиции **ML/LLM Engineer** в **hh.ru AI Lab**  
> Демонстрирует: LLM-агенты, RAG, fine-tuning, FastAPI production-сервис, eval pipeline

---

## 1. Соответствие вакансии hh.ru AI Lab

| Требование из вакансии | Что демонстрирует проект | Файл |
|---|---|---|
...
### 4.5 RAG Слой (планируется)

#### `api/rag/retriever.py` — гибридный поиск
```
Qdrant коллекция "vacancies"
├── Dense vector: BAAI/bge-small-en-v1.5 (768d)  — семантическое сходство
└── Sparse vector: BM42 (fastembed)               — точное совпадение терминов

Hybrid search = RRF(dense_results, sparse_results)
Зачем гибрид: dense хорошо находит синонимы ("ML" ≈ "machine learning"),
              sparse точно находит "LangGraph" как термин
```

#### `api/rag/indexer.py` — индексация вакансий
```
scripts/index_vacancies.py  →  читает JSON с вакансиями
                            →  вызывает indexer
                            →  загружает в Qdrant с метаданными
```
...
```
Неделя 1 (DONE ✅):
  api/agents/nodes/parse.py    — LLM парсинг
  api/agents/nodes/gap.py      — gap анализ
...
Неделя 2 (СЛЕДУЮЩАЯ):
  api/ml/skill_extractor.py    — BERT NER
  api/ml/seniority_clf.py      — DistilBERT + LoRA
  api/rag/retriever.py         — Qdrant hybrid search
  api/rag/indexer.py           — индексация вакансий
  scripts/index_vacancies.py   — CLI скрипт
