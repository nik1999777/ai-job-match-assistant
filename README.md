...
**Eval (12 тестовых кейсов, Llama 4 Scout):** match_in_range **100%** · MAE **0.000** · skill_F1 **0.817** · latency ~3.3s

**Advice:** структурированный вывод (Pydantic SeekerAdvice / HRAdvice) — skill badges, decision chip, нет стены текста  
**Similar vacancies:** RAG top-3 + skill benchmark chart + компания + зарплата (из Qdrant)  
**Qdrant:** 46 специализаций · 3-tier skill extraction (key_skills API + BERT NER + regex)
...
# ~4600 вакансий, 46 специализаций (~2 часа в фоне)
python -m scripts.index_vacancies --pages 1 --pause 1.5

# Кастомные запросы
python -m scripts.index_vacancies --query "Python Backend" --query "FastAPI" --pages 2
```

Скрипт работает в фоне и не мешает разработке. Прогресс: `tail -f /tmp/indexing.log`  
Данные: title, company, skills (3-tier extraction), salary, URL → Qdrant payload.
