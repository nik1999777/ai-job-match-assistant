...
_LOCAL_MODEL = Path(__file__).parent.parent.parent / "ml" / "models" / "ner"
NER_MODEL = str(_LOCAL_MODEL) if _LOCAL_MODEL.exists() else "dslim/bert-base-NER"
# Fine-tuned model outputs "SKILL"; base model outputs "MISC"/"ORG"
_VALID_GROUPS = frozenset(["SKILL"]) if _LOCAL_MODEL.exists() else frozenset(["MISC", "ORG"])
...
def _get_ner_pipeline():
    ...
    return pipeline("ner", model=NER_MODEL, aggregation_strategy="first")
...
def _extract_chunk(ner, text: str, seen: set[str], skills: list[str]) -> None:
    ...
    for e in entities:
        word = e["word"].strip()
        if (
            e["entity_group"] in _VALID_GROUPS
